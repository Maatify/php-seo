...
CI runs Consumer Verification on PHP 8.2 and 8.5 against the pinned MySQL fixture. **CI Gate** requires both Consumer Verification matrix jobs, in addition to the existing quality, standalone, lowest-dependency, persistence, and workflow-lint jobs. Consumer Verification provides evidence for the consumer-installation and runtime-integration contract it exercises; passing it does not prove external package publication or Stable eligibility. Release readiness, tagging, and publication remain separately governed by the applicable adopted package and repository standards.
