...
This document explains the current architecture, responsibilities, and practical implementation guidance for Structured Data (JSON-LD) within the Maatify SEO library.
...
### Scalar Compatibility and Explicit Offers
Scalar methods like `setPrice()` and `setCurrency()` on `ProductJsonLdBuilder` maintain an implicit `Offer` array for compatibility. The builder also provides an **Explicit Offers API** for structured offers such as `AggregateOffer` or multiple sellers:
...
-   Provides read-only JSON-LD validation through `SeoMetaValidator::validate(...)`, including generic node/list/`@graph` structural checks and scoped structural and property-range semantic validation for `Product`, `Offer`, `AggregateOffer`, and `ProductGroup`.
...
-   **Scoped Validation Only:** The validator provides generic structural checks and scoped structural and property-range semantic validation for the four selected types above. It is not complete Schema.org semantic or lexical proof, does not deeply validate every Schema.org type, and does not check URL reachability.
-   **Representation Is Not Lexical Proof:** In the current property-range boundaries, non-empty strings remain accepted for `URL`, `Date`, `DateTime`, `ItemAvailability`, and `OfferItemCondition`. The validator does not add URL grammar, date/datetime grammar, enumeration membership, provider-vocabulary lookup, or network/DNS validation.
-   **Unknown Schema.org Scope Is Preserved:** A well-formed `@type` outside the four selected types and unknown extension properties are not rejected merely for being outside the current catalog. Valid out-of-scope relationship targets are not deep-validated unless the existing traversal contract reaches an in-scope node.
-   **No Google Eligibility Profile:** Generic Schema.org builders and `SchemaGeneratorService` generation are independent of Google Rich Results, Course, Book Actions, or Merchant eligibility. Google required/recommended properties and provider constraints are not implemented here; generation does not guarantee eligibility.
-   **Course and Book Nuance:** The older Course Info search appearance was removed, while Course List documentation remains a separate provider context; this does not authorize Course eligibility validation. Book / Book Actions documentation exists, but Book Actions are provider-specific and are not a generic eligibility contract for every Book page. `CourseJsonLdBuilder` and `BookJsonLdBuilder` therefore remain callable generic Schema.org builders without provider rules.
-   **No Merchant Eligibility Validation:** Merchant eligibility is not implemented in this structured-data boundary. A complete Google/provider capability matrix and any future runtime eligibility profile require a separately approved, date-stamped contract with feature, property, constraint, source, and verification-date coverage.
